...
Gmail 和 Outlook Email 是可选的独立 App 连接器。需要邮件上下文或邮箱动作时，业务员在 Codex 中选择自己的邮箱并按平台流程完成授权；邮箱授权不进入 HollyDare CRM。连接器实际只开放读取、草稿、自投递或外部发送中的哪些能力，以当前 Codex 返回的工具为准，插件不会把草稿误报为已发送。
...
- 按需连接 Gmail 或 Outlook Email，读取真实邮件上下文，并在连接器实际开放相应动作时创建草稿或执行经确认的邮箱动作

远程 MCP 地址：`https://hollydare.cloud/api/mcp`

## 安全边界

- 插件连接 HollyDare 官方远程 MCP，并声明可选的 Gmail 与 Outlook Email App 连接器；各连接器独立授权。
